...
export async function GET(request: NextRequest) {
  try {
    ...
    const url = `${apiUrl}/customers${queryString ? `?${queryString}` : ''}`;
    ...
    if (!res.ok) {
      return NextResponse.json(
        { success: false, error: json?.message || 'เกิดข้อผิดพลาดในการดึงข้อมูลลูกค้า' },
        { status: res.status }
      );
    }
...
export async function POST(request: NextRequest) {
  try {
    ...
    const json = await res.json();
    if (!res.ok) {
      const msg = Array.isArray(json?.message) ? json.message.join(', ') : (json?.message || 'เกิดข้อผิดพลาดในการสร้างข้อมูลลูกค้า');
      return NextResponse.json(
