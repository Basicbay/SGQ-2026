...
export async function GET(request: NextRequest) {
  try {
    ...
    const url = `${apiUrl}/quotations${queryString ? `?${queryString}` : ''}`;
    ...
    if (!res.ok) {
      return NextResponse.json(
        {
          success: false,
          error: json?.message || 'เกิดข้อผิดพลาดในการดึงข้อมูลใบเสนอราคา',
        },
        { status: res.status },
      );
    }
  ...
  try {
    ...
    if (!res.ok) {
      const msg = Array.isArray(json?.message)
        ? json.message.join(', ')
        : json?.message || 'เกิดข้อผิดพลาดในการสร้างใบเสนอราคา';
      return NextResponse.json(
