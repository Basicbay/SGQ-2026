...
export async function GET(
  ...
  try {
    ...
    const res = await fetch(`${apiUrl}/quotations/${id}`, {
      headers,
      cache: 'no-store',
    ...
    if (!res.ok) {
      return NextResponse.json(
        {
          success: false,
          error: json?.message || 'ไม่พบข้อมูลใบเสนอราคา',
        },
        { status: res.status },
      );
    }
  ...
  try {
    ...
    if (!res.ok) {
      const msg = Array.isArray(json?.message)
        ? json.message.join(', ')
        : json?.message || 'เกิดข้อผิดพลาดในการอัปเดตใบเสนอราคา';
      return NextResponse.json(
  ...
  try {
    ...
    if (!res.ok) {
      return NextResponse.json(
        {
          success: false,
          error: json?.message || 'เกิดข้อผิดพลาดในการลบใบเสนอราคา',
        },
        { status: res.status },
      );
    }
