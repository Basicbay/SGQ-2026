  ...
  try {
    ...
    if (!res.ok) {
      return NextResponse.json(
        { success: false, error: json?.message || 'ไม่พบข้อมูลโครงการ' },
        { status: res.status }
      );
    }
  ...
  try {
    ...
    if (!res.ok) {
      const msg = Array.isArray(json?.message)
        ? json.message.join(', ')
        : (json?.message || 'เกิดข้อผิดพลาดในการอัปเดตข้อมูลโครงการ');
      return NextResponse.json(
...
export async function DELETE(
  ...
  try {
    ...
    if (!res.ok) {
      return NextResponse.json(
        { success: false, error: json?.message || 'เกิดข้อผิดพลาดในการลบข้อมูลโครงการ' },
        { status: res.status }
      );
    }
