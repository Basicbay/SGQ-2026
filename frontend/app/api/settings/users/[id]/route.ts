...
export async function GET(
  ...
  try {
    ...
    const res = await fetch(`${apiUrl}/settings/users/${id}`, {
      headers,
      cache: 'no-store',
    ...
    if (!res.ok) {
      return NextResponse.json(
        { success: false, error: json?.message || 'ไม่พบข้อมูลผู้ใช้งาน' },
        { status: res.status }
      );
    }
...
export async function PUT(
  ...
  try {
    ...
    const json = await res.json();
    if (!res.ok) {
      const msg = Array.isArray(json?.message) ? json.message.join(', ') : (json?.message || 'เกิดข้อผิดพลาดในการอัปเดตข้อมูลผู้ใช้งาน');
      return NextResponse.json(
...
export async function DELETE(
  ...
  try {
    ...
    if (!res.ok) {
      return NextResponse.json(
        { success: false, error: json?.message || 'เกิดข้อผิดพลาดในการลบผู้ใช้งาน' },
        { status: res.status }
      );
    }
