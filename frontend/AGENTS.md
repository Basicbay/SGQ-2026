...
- The minimum font size throughout the application is `text-xs`.
- Never use a font size smaller than `text-xs`.
- Never use arbitrary font-size utilities such as `text-[11px]`, `text-[10px]`, `text-[0.7rem]`, or equivalent inline styles and custom CSS.
- Use `text-xs` for captions, helper text, badges, table metadata, timestamps, validation messages, and other small text.
- When modifying existing UI, replace font sizes smaller than `text-xs` within the affected scope.
...
- Review all changed UI files and confirm that no font size smaller than `text-xs` is used.
- Remove arbitrary font-size utilities such as `text-[11px]` from the affected scope before completion.
...
- Verify that changed UI files contain no Tailwind utilities, inline styles, CSS declarations, or design tokens that render text smaller than `text-xs`.